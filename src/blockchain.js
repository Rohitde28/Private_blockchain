...
class Blockchain {
  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const curTime = parseInt(this.getUTCTimeStamp());

      const timeElapsed = 5 * 60;
      if (curTime - time > timeElapsed) {
        reject(new Error("Time elapsed is more than 5 minutes"));
      }

      const valid = bitcoinMessage.verify(message, address, signature);
      if (!valid) {
        reject(new Error("Verification has failed"));
      }

      try {
        const block = new BlockClass.Block({ star: star, owner: address });
        const response = await this._addBlock(block);
        resolve(response);
      } catch (e) {
        reject(e);
      }
